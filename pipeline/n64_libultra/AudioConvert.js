...
async function convertSoundBank(sourceDir, name, outputDir, includeDir, archive) {
    ...
    try {
        ...
        archive.addSoundBank(soundBankPath, name);
...
async function convertMusicBank(sourceDir, name, outputDir, includeDir, archive) {
    if (!fs.existsSync(sourceDir)) {
        throw new Error(`Source directory: ${sourceDir} does not exist`);
    }
    ...
    try {
        ...
        let index = -1;
        archive.withoutDefiningAssets(() => {
            index = archive.addFile(instrumentBankPath, name);
        });
        ...
        await archive.addMusicBank(musicBankPath, name);
    }
    catch (e) {
        console.log(e);
        console.log("docker", conversionArgs.join(' '));

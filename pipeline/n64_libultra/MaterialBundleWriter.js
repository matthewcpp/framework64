...
class N64LibUltraMaterialBundleWriter {
    _imageProcessor;
    ...
    async createImages(gltfData) {
        ...
        for (const imageJson of gltfData.images) {
            n64Images.push(await this._imageProcessor.convertWithoutBundling(imageJson, gltfDir));
        }

        return n64Images;
    }

    write(materialBundle, n64Images, gltfData, file) {
        this._computeN64TextureMasks(materialBundle, gltfData, n64Images);

        this._writeMaterialBundleInfo(materialBundle, file);
    ...
    _computeN64TextureMasks(materialBundle, gltfData, n64Images) {
        for (const textureIndex of materialBundle.textures) {
            const texture = gltfData.textures[textureIndex];
            const bundledImageIndex = materialBundle.getBundledImageIndex(texture.image);
    ...
    _writeMaterialBundleInfo(materialBundle, file){
...
module.exports = N64LibUltraMaterialBundleWriter;

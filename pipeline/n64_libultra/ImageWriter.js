...
class N64LibUltraImageWriter {
    writeFile(image, horizontalSlices, verticalSlices, path) {
        const file = fs.openSync(path, "w");
        fs.writeSync(file, this._buildImageBuffer(image, horizontalSlices, verticalSlices));
        fs.closeSync(file);
    }

    writeBuffer(image, horizontalSlices, verticalSlices) {
        return this._buildImageBuffer(image, horizontalSlices, verticalSlices);
    }

    _buildImageBuffer(image, horizontalSlices, verticalSlices) {
        ...
        buffers.push(this._createImageHeaderBuffer(image, horizontalSlices, verticalSlices));

        switch (image.format) {
            case Image.Format.IA8:
                for (const slice of slices.images) {
                    buffers.push(this._encodeIA8Slice(slice, sliceWidth, sliceHeight));
                }
                break;

            case Image.Format.IA4:
                for (const slice of slices.images) {
                    buffers.push(this._encodeIA4Slice(slice, sliceWidth, sliceHeight));
                }
                break;

            case Image.Format.RGBA16:
                for (const slice of slices.images) {
                    buffers.push(this._encode16bppSlice(slice, sliceWidth, sliceHeight));
                }
                break;

            case Image.Format.RGBA32:
                for (const slice of slices.images) {
                    buffers.push(this._encodeRGBA32Slice(slice, sliceWidth, sliceHeight));
                }
                break;

            case Image.Format.I8:
                for (const slice of slices.images) {
                    buffers.push(this._encodeI8Slice(slice, sliceWidth, sliceHeight));
                }
                break;

            case Image.Format.I4:
                for (const slice of slices.images) {
                    buffers.push(this._encodeI4Slice(slice, sliceWidth, sliceHeight));
                }
                break;

            case Image.Format.CI8:
                this._validateCIPalette(image);
                for (const slice of slices.images) {
                    buffers.push(this._encodeCI8Slice(slice, sliceWidth, sliceHeight, image.colorIndexImage));
                }
                buffers.push(this._buildColorIndexPalettesBuffer(image));
                break;

            case Image.Format.CI4:
                this._validateCIPalette(image);
                for (const slice of slices.images) {
                    buffers.push(this._encodeCI4Slice(slice, sliceWidth, sliceHeight, image.colorIndexImage));
                }
                buffers.push(this._buildColorIndexPalettesBuffer(image));
                break;
        }
    ...
    _createImageHeaderBuffer(image, horizontalSlices, verticalSlices) {
    ...
    _encode16bppSlice(data, width, height) {
    ...
    _encodeRGBA32Slice(data, sliceWidth, sliceHeight) {
    ...
    _encodeIA8Slice(data, sliceWidth, sliceHieght) {
    ...
    _encodeIA4Slice(data, sliceWidth, sliceHieght) {
    ...
    _encodeI8Slice(data, sliceWidth, sliceHieght) {
        const textelCount = sliceWidth * sliceHieght;
        const buffer = Buffer.alloc(textelCount);
    ...
    _encodeI4Slice(data, sliceWidth, sliceHieght) {
        const textelCount = (sliceWidth * sliceHieght) / 2;
        const buffer = Buffer.alloc(textelCount);
    ...
    _encodeCI8Slice(slice, sliceWidth, sliceHeight, colorIndexImage) {
    ...
    _encodeCI4Slice(slice, sliceWidth, sliceHeight, colorIndexImage) {
    ...
    _buildColorIndexPalettesBuffer(image) {
        const colorIndexImage = image.colorIndexImage;
...
module.exports = N64LibUltraImageWriter;

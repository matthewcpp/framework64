...
void star_init(Star* star, fw64Node* node, Player* player, StarTriggerFunc trigger_func, void* trigger_arg){
    ...
    star->initial_pos = node->transform.position;
    star->initial_scale = node->transform.scale;
}
...
void star_reset(Star* star) {
    star->triggered = 0;
    star->rotate.node->transform.position = star->initial_pos;
    star->rotate.node->transform.scale = star->initial_scale;
}

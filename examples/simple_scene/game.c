...
void game_init(Game* game, fw64Engine* engine) {
    ...
    // setup camera for third person controller
    fw64Node* camera_node = fw64_scene_create_node(game->scene);
    fw64_camera_init(&game->camera, camera_node, fw64_displays_get_primary(engine->displays));
    game->camera.near = 1.0f;
    game->camera.far = 100.0f;
    fw64_camera_update_projection_matrix(&game->camera);

    player_init(&game->player, game->engine, game->scene, &game->camera, FW64_ASSET_mesh_penguin, NULL);

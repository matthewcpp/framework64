...
static const fw64CharacterAnimationIds ids = {
    ...
    character_animation_LadderClimb,
    character_animation_LadderExit,
    character_animation_LadderEnterTop
};
...
void player_init(Player* player, fw64Engine* engine, fw64CharacterEnvironment* env, fw64Scene* scene, fw64Node* node, fw64Node* camera_node, fw64Allocator* allocator) {
    ...
    player->camera.far = 150.0f;
    fw64_camera_update_projection_matrix(&player->camera);
    ...
    fw64_third_person_camera_init(&player->third_person_cam, &player->camera, &player->character, &player->animation_controller);
    ...
    player->character.sphere_query_radius = fw64_maxf(player->character.size.x, player->character.size.z);
...
void player_on_animation_state_changed(fw64CharacterAnimationController* controller, fw64AnimationId prev, void* arg) {
    ...
    switch (controller->animation_state) {
        ...
        case character_animation_LadderExit:
            controller->skinned_mesh_instance->controller.speed = 1.5f;
            break;

...
void fw64_level_draw_frustrum(fw64Level* level, fw64RenderPass* rendererpass, fw64Frustum* frustum, uint32_t layer_mask) {
    for (uint32_t i = 0; i < level->chunk_ref_count; i++) {
        fw64LevelChunckRef* ref = level->chunk_refs + i;
        ...
        fw64_scene_draw_frustrum(ref->scene, rendererpass, frustum, layer_mask);
    }
}

...
fw64AnimationData* fw64_animation_data_load_from_datasource(fw64DataSource* data_source, fw64Allocator* allocator) {
    fw64AnimationData* data = fw64_allocator_malloc(allocator, sizeof(fw64AnimationData));
    ...
    char* data_ptr = fw64_allocator_malloc(allocator, info.joint_hierarchy_size);
    ...
    data->skin.inverse_bind_matrices = fw64_allocator_malloc(allocator, inv_bind_matrices_size);
    ...
    data->animations = fw64_allocator_malloc(allocator, sizeof(fw64Animation) * info.animation_count);
    fw64_data_source_read(data_source, data->animations, sizeof(fw64Animation), info.animation_count);

    data->tracks = fw64_allocator_malloc(allocator, info.track_data_size);
    fw64_data_source_read(data_source, data->tracks, 1, info.track_data_size);

    data->keyframe_data = fw64_allocator_malloc(allocator, info.keyframe_data_size);
...
void fw64_animation_data_delete(fw64AnimationData* data, fw64Allocator* allocator) {
    fw64_allocator_free(allocator, data->skin.joints); // note: joint hierarchy consists of single allocation
    fw64_allocator_free(allocator, data->skin.inverse_bind_matrices);
    fw64_allocator_free(allocator, data->animations);
    fw64_allocator_free(allocator, data->tracks);
    fw64_allocator_free(allocator, data->keyframe_data);
    fw64_allocator_free(allocator, data);
}

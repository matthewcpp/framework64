...
fw64RenderPass* fw64_renderpass_create(fw64Display* display, fw64Allocator* allocator) {
    ...
    memset(&renderpass->clear_color, 0, sizeof(Vec4));
...
void fw64_renderpass_set_clear_color(fw64RenderPass* renderer, uint8_t r, uint8_t g, uint8_t b) {
    vec4_set(&renderer->clear_color, (float)r / 255.0f, (float)g / 255.0f, (float)b / 255.0f, 1.0f);
}
...
void fw64_renderpass_set_camera(fw64RenderPass* renderpass, const fw64Camera* camera) {

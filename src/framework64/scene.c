...
int fw64_scene_partition_overlap_box(fw64SceneParition* partition, const Box* box, uint32_t layer_mask, fw64OverlapBoxQueryResult* result) {
    return partition->overlap_box(partition, box, layer_mask, result);
}
int fw64_scene_partition_overlap_sphere(fw64SceneParition* partition, const Vec3* center, float radius, uint32_t layer_mask, fw64OverlapSphereQueryResult* result) {
    return partition->overlap_sphere(partition, center, radius, layer_mask, result);
}

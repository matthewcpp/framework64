...
void fw64_assets_close_datasource(fw64AssetDatabase* assets, fw64DataSource* datasource) {
    (void)assets;
...
fw64Scene* fw64_assets_load_scene_with_options(fw64AssetDatabase* asset_database, fw64AssetId asset_id, const fw64SceneLoadOptions* options, fw64Allocator* allocator) {
    ...
    fw64Scene* scene = fw64_scene_load_from_datasource(data_source, asset_database, options, allocator);
